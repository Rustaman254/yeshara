...
const darkInput =
  "w-full rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm text-neutral-100 outline-none focus:border-violet-500";
      ...
      <div className="mx-auto max-w-lg">
        ...
        {kycStatus === "unverified" || kycStatus === "rejected" ? (
          <form onSubmit={onSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-neutral-500 mb-1">Full legal name</label>
              <input required value={fullName} onChange={(e) => setFullName(e.target.value)} className={darkInput} />
            </div>
            <div>
              <label className="block text-xs font-medium text-neutral-500 mb-1">Country of residence</label>
              <input required placeholder="KE" value={country} onChange={(e) => setCountry(e.target.value)} className={darkInput} />
            </div>
            <div>
              <label className="block text-xs font-medium text-neutral-500 mb-1">Residential address</label>
              <input required value={address} onChange={(e) => setAddress(e.target.value)} className={darkInput} />
            </div>
            <div>
              <label className="block text-xs font-medium text-neutral-500 mb-1">ID / passport number</label>
...
function StatusBanner({ status }: { status: api.KycStatus | null }) {
  ...
  const map: Record<api.KycStatus, { icon: React.ReactNode; text: string; cls: string }> = {
    unverified: { icon: <ShieldAlert className="h-4 w-4" />, text: "Not yet submitted.", cls: "bg-white/5 text-neutral-400 border-white/10" },
    pending: { icon: <Clock className="h-4 w-4" />, text: "Submitted — awaiting review.", cls: "bg-amber-500/10 text-amber-300 border-amber-500/20" },
    verified: { icon: <ShieldCheck className="h-4 w-4" />, text: "Verified. You can invest in live offerings.", cls: "bg-emerald-500/10 text-emerald-300 border-emerald-500/20" },
    rejected: { icon: <ShieldX className="h-4 w-4" />, text: "Rejected — please resubmit with correct details.", cls: "bg-red-500/10 text-red-300 border-red-500/20" },
  };
  const s = map[status];
  return (
